...
import { isFresh } from './capacityAtlas'
...
import { createCityLabels, capacityLabelText, labelAnchor, type CityLabels } from './cityLabels'
import type { AtlasSnapshot, CapacityAtlasItem, CapacityLink } from './fabricContracts'
...
/**
 * The server atlas: one small city per capacity on a shared grid.
 *
 * A capacity is a city here and a city again when it is entered, so the two surfaces read as two
 ...
 * shape, setbacks, and masts are decoration seeded from the capacity's stable id.
 *
 * Every city is named on the ground with the vocabulary the capacity city already uses. Without names
 * an atlas of a hundred cities can only be read by hovering each one in turn, which is not reading a
 * map.
 */

/**
 * Label height in world units for the atlas. Larger than the capacity city's, because the atlas frames
 ...
 * The capacity city one level down stands under the same hour, and an atlas lit differently from
...
export class AtlasScene {
   ...
   * thirty-second timer and a capacity's size has usually not moved, so rebuilding several thousand
   * boxes every refresh would be pure churn.
   */
  private readonly geometryCache = new Map<string, AtlasCityGeometry>()
  /** Materials carrying the selection highlight for one capacity. */
  ...
  constructor(canvas: HTMLCanvasElement, callbacks: AtlasSceneCallbacks) {
     ...
     * the same hour the capacity city one level down is drawn under.
     */
  ...
  setSnapshot(snapshot: AtlasSnapshot): void {
    ...
    const layout = this.layout.place(snapshot.capacities.map(capacity => capacity.capacityId))
    for (const capacity of snapshot.capacities) {
      const position = layout.get(capacity.capacityId)
      if (!position) continue
      const center = new THREE.Vector3(position.x, 0, position.z)
      centers.set(capacity.capacityId, center)
      const placed = this.addDatabase(capacity, center, snapshot.generatedAt)
      plans.set(capacity.capacityId, placed.plan)
    ...
    for (const edge of snapshot.links) {
      const from = centers.get(edge.fromCapacityId)
      const to = centers.get(edge.toCapacityId)
      if (!from || !to) continue
      this.addEdge(
        from,
        to,
        plans.get(edge.fromCapacityId),
        plans.get(edge.toCapacityId),
        edge.confidence,
        `${edge.fromCapacityId}->${edge.toCapacityId}`,
      )
    }
   ...
   *
   * The atlas holds one parcel per capacity, so a rebuild is cheaper and far simpler than a material
  ...
  private buildTerrain(): void {
    ...
    for (const patch of this.terrainPlan.patches) {
      ...
      // treatment the river bank gets, and the same one the capacity city gives its water.
  ...
  /** Places one capacity's city and returns its plan plus the cache signature its geometry was built under. */
  private addDatabase(
    capacity: CapacityAtlasItem,
    center: THREE.Vector3,
    generatedAt: string,
  ): { plan: AtlasCityPlan; signature: string } {
    const plan = planAtlasCity(capacity)
    ...
    const tint = plan.sizeKnown ? colorFor(capacity.capacityId) : 0x637080
    const materials: THREE.MeshStandardMaterial[] = []
    const flat = this.viewMode === 'map'

    // The pad is always present, so it is what a pointer finds over a city with no massing at all.
    // In map mode it becomes the parcel itself: a light plate with the capacity's tint, the way a
    ...
    this.addCityMesh(geometry.pad, padMaterial, center, capacity.capacityId, true)
    ...
    if (geometry.massing) {
      ...
      this.addCityMesh(geometry.massing, massingMaterial, center, capacity.capacityId, true)
    }
    ...
    if (geometry.trim) {
      ...
      this.addCityMesh(geometry.trim, trimMaterial, center, capacity.capacityId, false)
    }
    ...
    this.cityMaterials.set(capacity.capacityId, materials)
    this.addLabel(capacity, plan, center)
    this.expandContentBounds(center, plan)

    if (!plan.sizeKnown) this.addUnknownMark(center)
    if (isFresh(capacity.cuConsumed.evidence, generatedAt)) {
      this.addBeacon(center, PAD_HEIGHT + (plan.towerHeight ?? 0), capacity.capacityId)
    }
  ...
  private addCityMesh(
    ...
    mesh.userData.capacityId = capacityId
    mesh.userData.atlasObject = true
    this.disposables.push(material)
    this.scene.add(mesh)
   ...
   */
  private addLabel(capacity: CapacityAtlasItem, plan: AtlasCityPlan, center: THREE.Vector3): void {
    const sprite = this.labels.make(capacityLabelText(capacity.displayName))
    if (!sprite) return
  ...
  private addBeacon(center: THREE.Vector3, height: number, capacityId: string): void {
    ...
    this.beacons.push({ mesh: beacon, phase: (stableHash(capacityId) % 1000) / 100 })
  }

  /**
   * Draws the road between two capacities that reference each other.
   *
   ...
   * treatments, and the bow is seeded from the pair of capacity ids so a given reference draws the
  ...
  private addEdge(
    ...
    confidence: CapacityLink['confidence'],
    seedKey: string,
  ): void {
  ...
  private readonly handlePointerMove = (event: PointerEvent): void => {
    ...
    const nextId = hit?.object.userData.capacityId as string | undefined
  ...
  /**
   * Enters the double-clicked capacity's city. The click handler has already selected it, so the
   * detail panel and the city view name the same capacity, and the panel's own button stays the
   * keyboard-reachable way to do this.
   */
  private readonly handleDoubleClick = (): void => {
    const capacityId = this.activation.activate()
    if (capacityId) this.callbacks.onOpen(capacityId)
  }
...
function colorFor(capacityId: string): number {
  const palette = [0x39c6a3, 0x45a7e6, 0xe9a84c, 0xb48be8, 0x57bd70, 0xde6f73, 0x7bb7b2, 0xd58cb7]
  return palette[stableHash(capacityId) % palette.length] ?? 0x45a7e6
}
