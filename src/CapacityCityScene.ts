...
export type DatabaseCitySceneController = {
   ...
   */
  setWaterMainBreaks(itemIds: readonly string[]): void
  ...
  focusObject(itemId: string): void
...
export function createDatabaseCityScene(
  ...
  function buildIncidents(markers: readonly IncidentMarker[]) {
    ...
    for (const marker of markers) {
      const lot = plan.lots.get(marker.itemId)
      if (!lot) continue
  ...
  function buildDisasters() {
    ...
    for (const itemId of fireObjectIds) {
       ...
       */
      const roof = flat ? 1.5 : (buildingTops.get(itemId) ?? 12)
      const size = disasterSize(footprint * 0.42)
      for (let i = 0; i < FLAMES_PER_FIRE; i += 1) {
        const spin = disasterSeed(itemId, i) * Math.PI * 2
        ...
        flameParts.push({
          ...
          size: size * (i === 0 ? 1 : 0.7),
          rise: 0,
          phase: disasterSeed(itemId, i + 11),
          period: 0.55 + disasterSeed(itemId, i + 23) * 0.35,
        })
      }
      for (let i = 0; i < SMOKE_PER_FIRE; i += 1) {
        smokeParts.push({
          x: lot.x + (disasterSeed(itemId, i + 31) - 0.5) * footprint * 0.3,
          z: lot.z + (disasterSeed(itemId, i + 41) - 0.5) * footprint * 0.3,
          baseY: roof + size * 0.8,
          size: size * 0.9,
          rise: size * 3.4,
          phase: i / SMOKE_PER_FIRE,
          period: 2.6 + disasterSeed(itemId, i + 53) * 0.9,
        })
      }
      animatedDisasters += 1
    }

    for (const itemId of waterMainObjectIds) {
      const lot = plan.lots.get(itemId)
      if (!lot) continue
      const size = disasterSize((lot.footprint ?? 11) * 0.24)
      for (let i = 0; i < JETS_PER_BREAK; i += 1) {
        const spin = disasterSeed(itemId, i + 67) * Math.PI * 2
        ...
        jetParts.push({
          ...
          rise: 0,
          phase: disasterSeed(itemId, i + 71),
          period: 0.9 + disasterSeed(itemId, i + 83) * 0.5,
        })
      }
      puddleParts.push({
  ...
  function tourFacts(): TourFacts {
    ...
    if (plan) for (const [itemId, lot] of plan.lots) lots.set(itemId, { x: lot.x, z: lot.z })
  ...
  function buildBuildings(objects: readonly CapacityCityItem[], cityPlan: CityPlan) {
    ...
    for (const object of objects) {
      const lot = cityPlan.lots.get(object.itemId)
      if (!lot) continue
      ...
      group.userData.itemId = object.itemId
      ...
      buildingTops.set(object.itemId, known ? geometry.height : 0)
      const tint = tints.get(lot.districtId)
      const isWeathered = weathered(object.itemId)
      ...
      body.userData.itemId = object.itemId
      ...
      if (known && geometry.windows) {
        ...
        windows.userData.itemId = object.itemId
        group.add(windows)
      ...
      if (known && geometry.trim) {
        ...
        trim.userData.itemId = object.itemId
        group.add(trim)
  ...
  return {
    ...
    setStaleStatsObjects(itemIds) {
      ...
      if (staleStatsObjectIds.size === itemIds.length &&
          itemIds.every(itemId => staleStatsObjectIds.has(itemId))) {
        return
      }
      staleStatsObjectIds = new Set(itemIds)
      if (plan) buildBuildings(currentObjects, plan)
      // Rebuilding the massing invalidated the roof heights the fires stand on.
      refreshDisasters()
      requestRender()
    },
    setFireObjects(itemIds) {
      ...
      if (fireObjectIds.size === itemIds.length &&
          itemIds.every(itemId => fireObjectIds.has(itemId))) {
        return
      }
      fireObjectIds = new Set(itemIds)
      refreshDisasters()
      requestRender()
    },
    setWaterMainBreaks(itemIds) {
      if (waterMainObjectIds.size === itemIds.length &&
          itemIds.every(itemId => waterMainObjectIds.has(itemId))) {
        return
      }
      waterMainObjectIds = new Set(itemIds)
      refreshDisasters()
      requestRender()
    },
    setLayers(next) {
