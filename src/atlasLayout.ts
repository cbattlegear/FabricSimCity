 ...
 * lattice, which is what lets it be generated without knowing which capacities exist while still never
 ...
 *
 * A capacity keeps whichever slot it is first given, but which slots get handed out first is a choice,
 * and scattering them over the full hundred is the wrong one: a server with eight capacities would spread
 ...
 * slots first keeps a small server compact and a large one no worse off, without any capacity ever
...
export class AtlasLayoutReservations {
  ...
  place(databaseIds: readonly string[]): ReadonlyMap<string, AtlasPosition> {
    ...
    if (this.slotsById.size + unseenIds.length > slots) {
      throw new RangeError(`Atlas layout supports at most ${slots} reserved capacity IDs`)
    }

    for (const id of unseenIds) {
      const slot = slotsByDistance.find(candidate => !this.occupied.has(candidate))
      if (slot === undefined) throw new RangeError(`Atlas layout supports at most ${slots} reserved capacity IDs`)
