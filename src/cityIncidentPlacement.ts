 ...
 *
 * Throttling is a relationship between two things, and the old pin drew it as a property of one: a
 * cone on the roof of whichever item the throttled operation named. That reads as "this building is
 ...
 * on a guessed road and a pin placed on the measured road between two named items are different
 * claims, and a reader who cannot tell them apart is being misled by the more confident one.
 *
 * - `sharedRoad` — both parties are loaded items and a drawn road runs between them. The pin sits
 *   at the midpoint of that road, which is the only placement here that is *about* the relationship.
 * - `objectRoad` — only the throttled item is known, so the pin sits on the busiest road that
 *   item is an endpoint of, at the point nearest its frontage. Still on the road; the road is
 *   chosen by captured operations rather than by what was actually throttled.
 * - `frontage` — no road touches the item at all, so the pin sits at its kerb: the access point
 *   the address book and the route already use. Never the roof.
 *
 * All of this is pure geometry over inputs the caller measured. Nothing here reads a live source.
 */
...
const BASIS_RATIONALE: Readonly<Record<IncidentPlacementBasis, string>> = {
  sharedRoad:
    'Pinned to the midpoint of the road between the two items this throttling names, which is the road the overload is on.',
  objectRoad:
    'Only the throttled item could be resolved, so the pin sits on the busiest road that item is an endpoint of, at the point nearest its frontage. The road is chosen by captured operations, not by what was throttled.',
  frontage:
    'No drawn road reaches this item, so the pin sits at its frontage — the same access point the route uses. No road is being claimed.',
}

/**
 * Chooses the pin position for one incident.
 *
 * `counterpartObjectIds` are the *other* loaded items the same incident named — the items the
 * responsible operations also touched, so the pin can sit on the road *between* them. Empty is the
 * normal case, because most operations name a single item and therefore have no counterpart to
 * draw a road to.
 *
 * Returns null only when the item has neither a road nor a frontage, which means the caller asked
 * about an item this page has not placed. A caller must drop the pin rather than invent one.
 */
