...
describe('planCity placement', () => {
  it('is independent of the order rows arrive in', () => {
    ...
    const shuffled = planCity(
      [...sampleCity()].sort((left, right) => left.itemId.localeCompare(right.itemId)).reverse(),
      options(),
    )
    for (const plan of [reversed, shuffled]) {
      ...
      for (const [itemId, lot] of forward.lots) {
        expect(plan.lots.get(itemId)).toEqual(lot)
      }
    }
  })
  ...
  it('keeps a building on the same lot when a later bounded page is appended', () => {
    ...
    for (const item of firstPage) {
      const before = planned.lots.get(item.itemId)!
      const after = withMorePages.lots.get(item.itemId)!
  ...
  it('keeps the street network identical while pages with per-page schema counts arrive', () => {
    ...
    const countsFor = (objects: readonly CapacityCityItem[]): CapacityCityWorkspace[] => {
      const counts = new Map<string, number>()
      for (const item of objects) counts.set(item.workspaceId, (counts.get(item.workspaceId) ?? 0) + 1)
      return [...counts.entries()].map(([workspaceId, count]) => ({
        workspaceId,
        name: workspaceId,
        neighborhoodOrdinal: Number(workspaceId.slice(-1)),
  ...
  it('fronts every lot onto a street it can be entered from', () => {
    ...
    for (const lot of plan.lots.values()) {
      ...
      expect(street, `${lot.itemId} fronts a street that was pruned`).toBeDefined()
      ...
      expect(offStreet, `${lot.itemId} is entered from a point off its own street`)
        .toBeLessThan(1e-6)
        ...
        for (const point of street.path) {
          for (const lot of lots) {
            ...
            if (Math.abs(point.x - lot.x) < half && Math.abs(point.z - lot.z) < half) {
              violations.push(`${seed}: ${street.id} (${street.streetClass}) hits ${lot.itemId}`)
            }
          }
        }
      }
