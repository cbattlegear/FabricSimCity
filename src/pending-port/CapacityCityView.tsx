...
import { accessibleObjectLabel, attributedAbsenceLabel, databaseCityMetricValue, databaseCitySharedMetricValue, formatKiB, shouldRenderRoute } from './capacityCity'
import type { CapacityCityItem, CapacityCityPage, OperationFamily } from '../capacityCityContracts'
import type { LiveIncidentResponse } from '../liveContracts'
import type { LiveFeedConnectionState } from '../liveIncidents'
import type { NormalizedShowplan, QueryFamilySummary } from '../fabricContracts'
...
import { attributedWaits, type WaitAttributionTotals } from './cityThrottleAttribution'
...
export function CapacityCityView({ databaseId, databaseName, onBack, viewMode, onViewModeChange, banners }: Props) {
  const [metric, setMetric] = useState<Metric>('cpu')
  const [page, setPage] = useState<CapacityCityPage | null>(null)
  const [objects, setObjects] = useState<CapacityCityItem[]>([])
  ...
  const publish = useCallback((value: CapacityCityPage) => {
    setPage(value)
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let pages = 0
      let merged: CapacityCityPage | null = null
      try {
        do {
          ...
          if (pages === 1) {
            ...
            setSelectedId(current =>
              current && merged!.objects.some(object => object.itemId === current)
                ? current
                : merged!.objects[0]?.itemId ?? null)
            setLoading(false)
   ...
   */
  const familiesRef = useRef<readonly OperationFamily[]>([])
  /*
  ...
  const selectRoadEndpoint = useCallback((itemId: string) => {
    selectObject(itemId)
    restoreRoadFocus()
  ...
  const selected = objects.find(object => object.itemId === selectedId) ?? null
  /**
  ...
  const displayedSchemas = useMemo(() => {
    const byId = new Map<string, { workspaceId: string; name: string; neighborhoodOrdinal: number; itemCount: number }>()
    for (const object of objects) {
      const existing = byId.get(object.workspaceId)
      if (existing) existing.itemCount += 1
      else byId.set(object.workspaceId, {
        workspaceId: object.workspaceId,
        name: object.workspaceName,
        neighborhoodOrdinal: object.layout.neighborhoodOrdinal,
        itemCount: 1,
      })
    }
    return [...byId.values()].sort((left, right) =>
      left.neighborhoodOrdinal - right.neighborhoodOrdinal || left.workspaceId.localeCompare(right.workspaceId))
  }, [objects])
  ...
  const openIncidentFromList = useCallback((markerId: string) => {
    ...
    if (marker) {
      ...
      setIncidentFocus({ itemId: marker.itemId, nonce: incidentFocusNonce.current })
    }
  ...
  const roads = useMemo(() => {
    ...
    const visible = new Set(visibleObjects.map(object => object.itemId))
    return gradeRoads(
      page.routes.filter(candidate => shouldRenderRoute(candidate, visible)),
      page.topOperationFamilies,
      blocking.edges,
    )
  ...
  const endpointName = useCallback((itemId: string) => {
    const object = objects.find(candidate => candidate.itemId === itemId)
    return object ? `${object.workspaceName}.${object.name}` : itemId
  }, [objects])
  ...
  const roadLabels = useMemo(() => {
    ...
    for (const road of roads) {
      ...
      const mix = [road.fromItemId, road.toId]
        .map(itemId => {
          const label = facilityMixLabel(facilityShares(itemId, facilityTraffic))
          return label === null ? null : `${endpointName(itemId)} queued at ${label}`
        })
        .filter((entry): entry is string => entry !== null)
      labels.set(
        road.routeId,
        [
          `${endpointName(road.fromItemId)} ↔ ${endpointName(road.toId)}`,
          volume,
  ...
  const planOptions: CityPlanOptions = useMemo(
    () => ({ seed: databaseId, totalItems: page?.totalItems ?? null, schemas: planSchemas }),
    [databaseId, page?.totalItems, planSchemas],
  )
  ...
  const waitAttribution = useMemo(
    () => attributedWaits(families, new Set(visibleObjects.map(object => object.itemId))),
    [families, visibleObjects],
  )
   ...
   */
  const showFamilyOnMap = useCallback(async (family: OperationFamily) => {
  ...
  const addressEntries = useMemo(
    () => buildAddressBook(visibleObjects, page?.topOperationFamilies ?? [], facilities, cityPlan),
    [visibleObjects, page?.topOperationFamilies, facilities, cityPlan],
  )
  ...
  const openAddress = useCallback((entry: AddressEntry) => {
    if (entry.kind === 'query') {
      const family = page?.topOperationFamilies.find(candidate => candidate.familyId === entry.targetId)
  ...
  }, [page?.topOperationFamilies, selectObject, showFamilyOnMap])
  ...
  const placeCard = route
    ...
    : selectedRoad
      ? <RoadPanel
        road={selectedRoad}
        fromName={endpointName(selectedRoad.fromItemId)}
        toName={endpointName(selectedRoad.toId)}
        onSelectEndpoint={selectRoadEndpoint}
        hasEndpoint={itemId => objects.some(object => object.itemId === itemId)}
        onClose={closeRoad}
      />
      : selectedFacility
  ...
  const sidebarMode = resolveSidebarMode({
    databaseName,
    // `totalItems` is a decimal string because it can exceed `Number.MAX_SAFE_INTEGER`, so it is
    // passed through as it arrives rather than being routed through a lossy `Number`.
    totalObjectsLabel: page?.totalItems ?? '—',
    route: route
      ? {
              ...
              <div className="sidebar-metric">
                ...
                {backfilling && (
                  ...
                  <p className="load-progress">
                    ...
                    {Number(page?.totalItems ?? loadedCount).toLocaleString()} objects placed.
                  </p>
                )}
    ...
    <MapShell sidebar={sidebar} kiosk={kiosk}>
      ...
      {((loading && !page) || backfilling || relayouting) && (
        <CityLoadingScreen
          ...
          loaded={loadedCount}
          total={page?.totalItems != null ? Number(page.totalItems) : null}
        />
      )}
...
function LiveQueryTicker({
  ...
  onShowFamily: (family: OperationFamily) => void | Promise<void>
}) {
  const byId = useMemo(
...
function LegendDrawer({
  ...
  page: CapacityCityPage
  objects: readonly CapacityCityItem[]
  metric: Metric
  selectedId: string | null
  selectedRoadId: string | null
  onSelectObject: (itemId: string) => void
  onSelectRoad: (routeId: string) => void
  endpointName: (itemId: string) => string
  roads: readonly RoadTraffic[]
  facilities: readonly Facility[]
  facilityTraffic: FacilityTraffic
  waitAttribution: WaitAttributionTotals
  blocking: LiveBlockingSummary
  displayedSchemas: ReadonlyArray<{ workspaceId: string; name: string; neighborhoodOrdinal: number; itemCount: number }>
  activePlanFamilyId: string | null
  mappingFamilyId: string | null
  onShowFamily: (family: OperationFamily) => void | Promise<void>
  selectedObject: CapacityCityItem | null
  vehicles: VehicleRoster
  ...
  const trafficWindow = useMemo(
    () => describeTrafficWindow(page.topOperationFamilies, refreshedAt, CITY_REFRESH_INTERVAL_MS),
    [page.topOperationFamilies, refreshedAt],
  )
  const placedIds = useMemo(() => placedObjectIds(objects), [objects])
  const familySplit = useMemo(
    () => splitQueryFamiliesByMap(page.topOperationFamilies, placedIds, showUnmappedFamilies),
    [page.topOperationFamilies, placedIds, showUnmappedFamilies],
  )
  return (
    <details
      className="sidebar-drawer"
        ...
        <section className="table-region" aria-labelledby="city-object-table">
          ...
          <div className="table-scroll">
            <table>
              ...
              <tbody>{objects.map(object => <tr key={object.itemId} className={selectedId === object.itemId ? 'is-selected' : undefined}>
                <th scope="row"><button type="button" aria-label={accessibleObjectLabel(object)}
                  aria-pressed={selectedId === object.itemId} onClick={() => onSelectObject(object.itemId)}>
                  {object.workspaceName}.{object.name}
        ...
        <section className="city-workload" aria-labelledby="city-workload-title">
          <div className="section-heading">
            <div><h2 id="city-workload-title">Top query-family exposure</h2>
              <p>Backend-ranked top {page.topOperationFamilies.length}; no browser-side 100k layout</p></div>
          </div>
          {/*
          ...
          <div className="table-scroll"><table>
            ...
            <tbody>{familySplit.shown.map(family => <tr key={family.familyId}>
              <th scope="row">{family.familyId}<small>{family.familyId} · {family.evidence.source}</small></th>
              ...
              <td className="map-cell">{familyOnMap(family, placedIds) ? <>
                ...
                {/*
                  ...
                  */}
                {family.itemIds.length === 0
                  ? 'Its plans named no object in this database.'
                  : `Its plans named ${family.itemIds.length} object(s), none of them on this bounded page.`}
              </small></span>}</td>
            </tr>)}</tbody>
            ...
            return <li key={route.routeId} className={route.routeId === selectedRoadId ? 'is-selected' : undefined}>
              ...
              <span>
                {graded
                  ...
                  >
                    {endpointName(route.fromItemId)} ↔ {endpointName(route.toId)}
                  </button>
                  : <>{endpointName(route.fromItemId)} ↔ {endpointName(route.toId)}</>}
                <br />
                {graded
...
function WaitAttributionTable({
  ...
  const nameOf = (itemId: string) => {
    const object = objects.find(item => item.itemId === itemId)
    return object ? `${object.workspaceName}.${object.name}` : itemId
  }
  ...
  return (
    <section className="city-wait-attribution" aria-labelledby="city-wait-attribution-title">
      ...
      {rows.length > 0 && <div className="table-scroll"><table>
        ...
        <tbody>{rows.map(entry => <tr key={entry.itemId}>
          <th scope="row">{nameOf(entry.itemId)}</th>
    ...
    <section className="city-wait-lanes" aria-labelledby="city-wait-lanes-title">
      ...
      {traffic.lanes.length > 0 && <div className="table-scroll"><table>
        ...
        <tbody>{traffic.lanes.map(lane => <tr key={lane.laneId}>
          <th scope="row">{nameOf(lane.itemId)}<small>{lane.familyIds.join(' · ')}</small></th>
      ...
      {traffic.sharedLanes.length > 0 && <div className="table-scroll"><table>
        ...
        <tbody>{traffic.sharedLanes.map(lane => <tr key={lane.laneId}>
          ...
          <td>{lane.itemIds.map(nameOf).join(' · ')}
...
function RoadPanel({
  ...
  fromName: string
  toName: string
  onSelectEndpoint: (itemId: string) => void
  hasEndpoint: (itemId: string) => boolean
  onClose: () => void
}) {
  const endpoint = (itemId: string, name: string) =>
    hasEndpoint(itemId)
      ? <button type="button" className="link-button" onClick={() => onSelectEndpoint(itemId)}>{name}</button>
  ...
  return (
    <aside className="hud-slideover" aria-labelledby="city-road-title">
      ...
      <p className="road-endpoints">
        {endpoint(road.fromItemId, fromName)}
        <span aria-hidden="true"> ↔ </span>
...
function BuildingPanel({
  ...
  return (
    <aside className="hud-slideover" aria-labelledby="city-building-title">
      <div className="detail-title">
        <h2 id="city-building-title">{object.workspaceName}.{object.name}</h2>
        <span>{object.kind}</span>
      </div>
      <dl>
        <div><dt>Reserved pages</dt><dd>{object.storageBytes ?? object.sizeReason}</dd></div>
        <div><dt>Used pages</dt><dd>{object.cuSecondsRaw ?? object.sizeReason}</dd></div>
...
function ObjectDetail({ object, metric }: { object: CapacityCityItem | null; metric: Metric }) {
  ...
  return <aside className="detail city-object-detail" aria-labelledby="city-object-detail-title">
    <div className="detail-title"><h2 id="city-object-detail-title">{object.workspaceName}.{object.name}</h2><span>{object.kind}</span></div>
    <dl>
      <div><dt>Stable ID</dt><dd>{object.itemId}</dd></div>
      <div><dt>Reserved pages / bytes</dt><dd>{object.storageBytes ?? 'Unavailable'} / {object.reservedBytes ?? 'Unavailable'}</dd></div>
      <div><dt>Used pages / bytes</dt><dd>{object.cuSecondsRaw ?? 'Unavailable'} / {object.usedBytes ?? 'Unavailable'}</dd></div>
      <div><dt>Direct operations</dt><dd>{object.directActivity.totalOperations ?? object.directActivity.evidence.status}</dd></div>
