...
import type { NormalizedShowplan, QueryFamilyDetail, ShowplanNode } from '../fabricContracts'
...
const OBJECTS = [
  cityObject({ itemId: 'o1', workspaceName: 'app1', name: 'orders' }),
  cityObject({ itemId: 'o2', workspaceName: 'app1', name: 'invoices' }),
]

function family(familyId: string): OperationFamily {
  return { familyId } as unknown as OperationFamily
}
...
describe('surveyShowplan resolves a plan against the city that is actually loaded', () => {
  it('attaches a missing index to the building it names', () => {
    ...
    expect(read.missingIndexes[0].itemId).toBe('o1')
  ...
  it('keeps a suggestion that names a table this page does not hold, with no object to draw it on', () => {
    ...
    expect(read.missingIndexes).toHaveLength(1)
    expect(read.missingIndexes[0].itemId).toBeNull()
  ...
  it('matches a warning kind whatever case the plan spelled it in', () => {
    ...
    expect(read.warnings[0].itemId).toBe('o1')
  })
  ...
  it('falls back to an ancestor when nothing underneath names a table', () => {
    ...
    expect(read.warnings).toHaveLength(1)
    expect(read.warnings[0].itemId).toBe('o2')
  })
  ...
  it('reports a warning it cannot place rather than hiding it', () => {
    ...
    expect(read.warnings).toHaveLength(1)
    expect(read.warnings[0].itemId).toBeNull()
  })
})
...
describe('mergeDisasterSurvey', () => {
  ...
  it('counts one plan once, however many families led to it', () => {
    const survey = mergeDisasterSurvey(
      [
        evidenceFor({ planId: 'p1', familyId: 'f1', missingIndexes: [{ itemId: 'o1', label: 'app1.orders', impactPercent: 90, familyId: 'f1', planId: 'p1' }] }),
        evidenceFor({ planId: 'p1', familyId: 'f2', missingIndexes: [{ itemId: 'o1', label: 'app1.orders', impactPercent: 90, familyId: 'f2', planId: 'p1' }] }),
      ],
...
describe('runDisasterSurvey', () => {
  ...
  it('keeps going when one family has no plan and another cannot be read', async () => {
    ...
    expect(survey.missingIndexes.map(entry => entry.itemId)).toEqual(['o1'])
  })
  ...
  it('re-resolves a cached plan against the objects that have since paged in', async () => {
    ...
    expect(first.missingIndexes[0].itemId).toBeNull()

    const wider = [...OBJECTS, cityObject({ itemId: 'o9', workspaceName: 'app2', name: 'ledger' })]
    ...
    expect(second.missingIndexes[0].itemId).toBe('o9')
  ...
  it('retries a family whose plan fetch failed, rather than remembering the failure', async () => {
    ...
    expect(second.missingIndexes[0].itemId).toBe('o1')
  })
