import type { NormalizedShowplan, ShowplanNode, QueryFamilyDetail } from '../fabricContracts'
 ...
 *
 * The page's `topOperationFamilies` is usually shorter than this, in which case the survey reads all of
 ...
 *
 * A warning that resolves to nothing is still returned, with a null `itemId`. It is real, it is
  ...
  for (const node of nodes) {
    for (const warning of node.warnings ?? []) {
      ...
      warnings.push({
        ...
        label: matched ? `${matched.workspaceName}.${matched.name}` : carrier ? referenceLabel(carrier) : 'an unplaced operator',
        familyId,
...
export function surveyShowplan(
  showplan: NormalizedShowplan,
  {
    ...
    databaseName,
    familyId,
  }: { objects: readonly CapacityCityItem[]; databaseName: string; familyId: string },
