...
function statistics(
  overrides: Partial<NonNullable<CapacityCityItem['statistics']>> = {},
): CapacityCityItem['statistics'] {
  return {
...
describe('projectCityDisasters', () => {
  ...
  it('reports nothing when no statistics were measured, whatever the page evidence says', () => {
    const projection = projectCityDisasters({
      incidents: incidents(),
      route: null,
      objects: [cityObject({ itemId: 'unmeasured', statistics: undefined })],
    })
