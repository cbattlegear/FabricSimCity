...
export function projectCityDisasters({
  ...
  const waterMainObjectIds = [...new Set(
    allWarnings.map(warning => warning.itemId).filter((id): id is string => id !== null),
  )]
  ...
  const fireObjectIds = [...new Set(
    fires.map(fire => fire.itemId).filter((id): id is string => id !== null),
  )]
  if (fires.length > 0) {
 ...
 */
function staleStatsObjectIds(objects: readonly CapacityCityItem[]): string[] {
  const ids: string[] = []
  for (const object of objects) {
    ...
    if (past > 0) ids.push(object.itemId)
  }
  return ids
}
