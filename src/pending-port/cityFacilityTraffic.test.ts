...
const objects = [{ itemId: 'a' }, { itemId: 'b' }]
...
describe('laneWidth', () => {
  ...
  it('clamps rather than growing without bound, and flags the clamp as a floor', () => {
    ...
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'big', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '999999999999' } }),
        family({ familyId: 'small', itemIds: ['b'], waitMillisecondsByCategory: { 'Lock': '5000' } }),
      ],
      objects,
    )
...
describe('projectFacilityTraffic', () => {
  it('draws one lane per building and destination facility', () => {
    const traffic = projectFacilityTraffic(
      [family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Buffer IO': '400', 'Lock': '100' } })],
      objects,
    )
  ...
  it('draws a multi-object family as one shared lane instead of dividing it between its buildings', () => {
    const traffic = projectFacilityTraffic(
      [family({ familyId: 'f1', itemIds: ['a', 'b'], waitMillisecondsByCategory: { 'Lock': '1000' } })],
      objects,
    )
    ...
    expect(lane.itemIds).toEqual(['a', 'b'])
  ...
  it('threads a shared lane through only the named objects on this page, and says so', () => {
    ...
    const traffic = projectFacilityTraffic(
      [family({
        familyId: 'f1',
        itemIds: ['a', 'other-database/table'],
  ...
  it('reports a multi-object family whole in text when none of its objects are on this page', () => {
    const traffic = projectFacilityTraffic(
      [family({
        familyId: 'f1',
        itemIds: ['off-page', 'other-database/table'],
        waitMillisecondsByCategory: { 'Lock': '1000' },
      })],
      objects,
    )
  ...
  it('keeps a shared lane out of every per-object total', () => {
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'solo', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '10' } }),
        family({ familyId: 'pair', itemIds: ['a', 'b'], waitMillisecondsByCategory: { 'Lock': '900' } }),
      ],
      objects,
    )
    ...
    expect(traffic.lanes).toHaveLength(1)
    expect(traffic.lanes[0].itemId).toBe('a')
  ...
  it('keeps wait time from a family whose single named object is off this page', () => {
    const traffic = projectFacilityTraffic(
      [family({ familyId: 'f1', itemIds: ['off-page'], waitMillisecondsByCategory: { 'Memory': '75' } })],
      objects,
    )
  ...
  it('draws no lane when no category evidence was captured and says why', () => {
    const traffic = projectFacilityTraffic([family({ familyId: 'f1', itemIds: ['a'] })], objects)
  ...
  it('carries the weakest contributing attribution into the lane pattern', () => {
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '10' } }),
        family({
          familyId: 'f2',
          itemIds: ['a'],
          confidence: 'Probable',
  ...
  it('orders lanes by captured milliseconds so the ordering is stable and readable', () => {
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '10' } }),
        family({ familyId: 'f2', itemIds: ['b'], waitMillisecondsByCategory: { 'Memory': '500' } }),
      ],
      objects,
    )
  ...
  it('preserves exact counters far beyond IEEE-754 integer precision', () => {
    ...
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '9007199254740993' } }),
        family({ familyId: 'f2', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '2' } }),
      ],
      objects,
    )
...
describe('facilityShares · the readout that replaced the lanes', () => {
  ...
  it('answers for the object asked about and not for its neighbours', () => {
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '100' } }),
        family({ familyId: 'f2', itemIds: ['b'], waitMillisecondsByCategory: { 'Buffer IO': '900' } }),
      ],
      objects,
    )
  ...
  it('adds up several families that queued at the same facility', () => {
    const traffic = projectFacilityTraffic(
      [
        family({ familyId: 'f1', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '100' } }),
        family({ familyId: 'f2', itemIds: ['a'], waitMillisecondsByCategory: { 'Lock': '400' } }),
      ],
      objects,
    )
  ...
  it('leaves a multi-object family\'s wait out, rather than giving it to both', () => {
    const traffic = projectFacilityTraffic(
      [family({ familyId: 'f1', itemIds: ['a', 'b'], waitMillisecondsByCategory: { 'Lock': '900' } })],
      objects,
    )
