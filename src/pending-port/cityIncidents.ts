 ...
 * same request.
 *
 * Exported, rather than inlined at the one call site in `CapacityCityScene.ts`, so the rule can be
...
function objectKeys(object: CapacityCityItem): string[] {
  ...
  const numeric = numericObjectId(object.itemId)
 ...
 * `workspaceName`/`objectName` empty by design. So the name join those locks would need is never
 * populated, and a table-level block could only ever be matched numerically.
 */
function numericObjectId(itemId: string): string | null {
  const marker = itemId.lastIndexOf('/object/')
  if (marker < 0) return null
  const tail = itemId.slice(marker + '/object/'.length)
...
export function projectIncidents(
   ...
   *
   * The name join is tried first and needs no gate: `workspaceName`/`objectName` are only ever
  ...
  const noteSessionObject = (source: LiveRequest | WaitingTask) => {
    ...
    const itemId = resolveLockObject(lock, source.sessionId)
    if (itemId) objectBySession.set(source.sessionId, itemId)
  }
  ...
  const consider = (
    ...
    const details = [
      ...
      `lock resource ${lock.rawResource} resolved to ${lock.workspaceName ?? '?'}.${lock.objectName ?? '?'}`,
    ]
    if (inCycle) {
    ...
    const marker: IncidentMarker = {
      id: `${kind}:${source.sessionId}:${itemId}`,
      itemId,
      counterpartObjectIds: blockerObjectId && blockerObjectId !== itemId ? [blockerObjectId] : [],
    ...
    const existing = byObject.get(itemId)
    if (!existing) {
      byObject.set(itemId, marker)
      return
    }
  ...
  markers.sort((left, right) => left.itemId.localeCompare(right.itemId) || left.id.localeCompare(right.id))
...
function projectDeadlocks(
  ...
  for (const graph of sample.graphs) {
    ...
    const resolved = graph.resources
      .map(resource => ({ resource, itemId: resolveDeadlockObject(resource, byKey, inPageDatabase) }))
      .filter((entry): entry is { resource: DeadlockResource; itemId: string } => entry.itemId !== null)
    ...
    const counterparts = [...new Set(resolved.map(entry => entry.itemId))].filter(id => id !== anchor.itemId)
    ...
    markers.push({
      id: `deadlock:${graph.id}:${anchor.itemId}`,
