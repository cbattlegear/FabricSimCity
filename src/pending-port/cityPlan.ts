...
export function planCity(
  ...
  const capacity = plannedCount(
    Math.max(parseCount(options.totalItems) ?? 0, ordered.length),
  ...
  const groundwork = cityGroundwork(
    [
      ...
      capacity,
      widest,
      sizes.map(size => `${size.workspaceId}:${size.count}`).join(','),
    ].join('|'),
...
function placeBuildings(
  ...
  for (const [workspaceId, members] of bySchema) {
    ...
    arrivals.forEach((object, index) => {
      ...
      lots.set(object.itemId, placeLot(object, block))
...
function schemaSizes(
  ordered: readonly CapacityCityItem[],
  schemas: readonly CapacityCityWorkspace[] | undefined,
): SchemaSize[] {
  const counts = new Map<string, { ordinal: number; count: number }>()

  if (schemas && schemas.length > 0) {
    for (const schema of schemas) {
      counts.set(schema.workspaceId, {
        ordinal: schema.neighborhoodOrdinal,
        count: parseCount(schema.itemCount) ?? 0,
      })
    }
  }
  ...
  for (const object of ordered) {
    loaded.set(object.workspaceId, (loaded.get(object.workspaceId) ?? 0) + 1)
  }
  for (const object of ordered) {
    const existing = counts.get(object.workspaceId)
    const observed = loaded.get(object.workspaceId) ?? 1
    if (!existing) {
      counts.set(object.workspaceId, { ordinal: object.layout.neighborhoodOrdinal, count: observed })
    } else if (observed > existing.count) {
      counts.set(object.workspaceId, { ordinal: existing.ordinal, count: observed })
    }
  }

  return [...counts.entries()]
    .map(([workspaceId, entry]) => ({ workspaceId, ordinal: entry.ordinal, count: entry.count }))
    .sort((left, right) => left.ordinal - right.ordinal || compareOrdinal(left.workspaceId, right.workspaceId))
}
...
function planNeighborhoods(
  ...
  for (const schema of schemas) territories.set(schema.workspaceId, [])
  ...
  const claim = (index: number, block: CityBlock) => {
    ...
    territories.get(schemas[index].workspaceId)!.push(block)
  ...
  while (growing) {
    ...
    for (let index = 0; index < schemas.length; index += 1) {
      if (territories.get(schemas[index].workspaceId)!.length >= quotas[index]) continue
...
function describeDistricts(
  ...
  for (const object of ordered) {
    const lot = lots.get(object.itemId)
    if (!lot) continue
    const existing = groups.get(object.workspaceId)
    if (existing) {
      existing.lots.push(lot)
      existing.ordinal = Math.min(existing.ordinal, object.layout.neighborhoodOrdinal)
    } else {
      groups.set(object.workspaceId, {
...
function placeLot(object: CapacityCityItem, block: CityBlock): CityLot {
  /*
