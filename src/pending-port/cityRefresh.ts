 ...
 */
export function cityLayoutSignature(objects: readonly CapacityCityItem[]): string {
  return objects
    .map(object => [
      object.itemId,
      object.workspaceId,
 ...
 */
export function citySchemaSignature(schemas: readonly CapacityCityWorkspace[] | undefined): string {
  if (!schemas) return ''
  return schemas
    .map(schema => [schema.workspaceId, schema.name, schema.neighborhoodOrdinal, schema.itemCount].join('\u0000'))
    .join('\u0001')
}
