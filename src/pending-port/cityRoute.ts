import type { NormalizedShowplan, ShowplanNode, ShowplanObjectReference } from '../fabricContracts'
 ...
 * to no operator and cannot be read off a stop's warnings. `itemId` is null when the suggestion
...
export function matchObject(
  ...
  return (
    objects.find(object => `${object.workspaceName}.${object.name}`.toLowerCase() === wanted) ??
    (schema === null
...
interface StopKey {
  /** Stable identity: an itemId for a placed building, or a described reference for an off-map one. */
  readonly key: string
  readonly kind: StopKind
  readonly object: CapacityCityItem | null
...
function buildStops(showplan: NormalizedShowplan, context: RouteContext): Built {
  ...
  for (const node of showplan.nodes) {
    ...
    const key = matched ? matched.itemId : `offmap:${describeReference(node.objectReference)}`
  ...
  if (split.total > 0) {
    for (const entry of split.objects) {
      ...
      const key = matched ? matched.itemId : `offmap:${describeReference(entry.reference)}`
  ...
  order.forEach((key, index) => {
    ...
    const lot = entry.object ? context.plan.lots.get(entry.object.itemId) : undefined
    const label = entry.object
      ? `${entry.object.workspaceName}.${entry.object.name}`
      : describeReference(entry.reference)
    stops.push({
      ordinal: index + 1,
      kind: entry.kind,
      label,
      itemId: entry.object?.itemId ?? null,
      indexNames,
      x: lot?.accessX ?? null,
      z: lot?.accessZ ?? null,
      operations,
...
function operationInstruction(
  ...
  if (node.objectReference !== null) {
    const matched = matchObject(node.objectReference, context.objects, context.databaseName)
    const label = matched
      ? `${matched.workspaceName}.${matched.name}`
...
function stopLabel(stopKey: StopKey): string {
  return stopKey.object
    ? `${stopKey.object.workspaceName}.${stopKey.object.name}`
