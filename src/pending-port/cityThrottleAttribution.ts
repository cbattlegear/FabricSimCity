...
export function familyWaitByObject(family: OperationFamily): Map<string, bigint> {
  ...
  for (const entry of attribution.objects) {
    ...
    waits.set(entry.itemId, (waits.get(entry.itemId) ?? 0n) + milliseconds)
  }
  return waits
}

/** The estimated cost share a family's plans placed on each object it reads. */
export function familyCostShares(family: OperationFamily): Map<string, number> {
  const shares = new Map<string, number>()
  const attribution = familyAttribution(family)
  if (!attribution) return shares
  for (const entry of attribution.objects) {
    if (!Number.isFinite(entry.estimatedCostShare)) continue
    shares.set(entry.itemId, (shares.get(entry.itemId) ?? 0) + entry.estimatedCostShare)
  }
  return shares
}
...
export function attributedWaits(
  ...
  for (const family of families) {
    measured += parse(family.throttlingSeconds)
    const attribution = family.waitAttribution
    if (!attribution) {
      unattributed += parse(family.throttlingSeconds)
      continue
    }
    ...
    for (const entry of attribution.objects) {
      ...
      if (drawnObjectIds && !drawnObjectIds.has(entry.itemId)) {
        unattributed += milliseconds
        continue
      }
      const existing = byObject.get(entry.itemId)
      if (existing) {
        existing.milliseconds += milliseconds
        existing.familyIds.push(family.familyId)
      } else {
        byObject.set(entry.itemId, { milliseconds, familyIds: [family.familyId] })
      }
    }
  }

  const totals = new Map<string, BuildingWait>()
  for (const [itemId, entry] of byObject) {
    totals.set(itemId, { itemId, milliseconds: entry.milliseconds, familyIds: entry.familyIds })
  }
