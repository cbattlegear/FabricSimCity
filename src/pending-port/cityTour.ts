...
function objectLabel(object: CapacityCityItem): string {
  return `${object.workspaceName}.${object.name}`
}
...
export function planCityTour(facts: TourFacts, options: TourOptions = {}): readonly TourStop[] {
  ...
  const byId = new Map(facts.objects.map(object => [object.itemId, object]))
  ...
  const addLandmark = (object: CapacityCityItem, detail: string, rank: number) => {
    if (claimed.has(object.itemId)) return
    claimed.add(object.itemId)
    const lot = facts.lots.get(object.itemId)
    if (!lot) return
    const id = `tour:landmark:${object.itemId}`
  ...
  for (const object of withLot) {
    const lot = facts.lots.get(object.itemId)!
    const entry = schemas.get(object.workspaceId)
    if (entry) entry.points.push(lot)
    else schemas.set(object.workspaceId, { name: object.workspaceName, points: [lot] })
  }
  ...
  for (const [workspaceId, entry] of neighbourhoods) {
    const id = `tour:neighbourhood:${workspaceId}`
    const centroid = {
