 ...
 *
 * `familyId` arrives already normalized, because the feed normalizes it — the roster looks the
...
function volume(bytes: string): OperationFamily['planDataVolume'] {
  return {
    estimatedBytesPerExecution: bytes,
    byObject: [],
    plansRead: 1,
    rationale: 'test',
  } as unknown as OperationFamily['planDataVolume']
}
...
describe('the join is query_hash and only query_hash', () => {
  ...
  it('counts an unmatched hash instead of inventing a family for it', () => {
    const roster = buildVehicleRoster({
      events: [event({ familyId: '00FF00FF00FF00FF' })],
      families: [family()],
      roads: [road()],
      blocked: noBlocks,
      now: NOW,
  ...
  it('normalizes the family side of the join, so a rendering difference cannot empty the roads', () => {
    const roster = buildVehicleRoster({
      events: [event()],
      families: [family({ familyId: '0xaabbccddeeff0011' })],
      roads: [road()],
      blocked: noBlocks,
      now: NOW,
    })
  ...
  it('rejects the all-zero sentinel rather than making it a family everyone shares', () => {
    ...
    const roster = buildVehicleRoster({
      ...
      events: [event({ familyId: null })],
      families: [family({ familyId: '0000000000000000' })],
      roads: [road()],
      blocked: noBlocks,
      now: NOW,
    })
  ...
  it('uses the family the feed already resolved rather than re-deriving one', () => {
    const roster = buildVehicleRoster({
      events: [event({ familyId: 'fam-2', familyId: 'AABBCCDDEEFF0011' })],
      families: [family(), family({ familyId: 'fam-2', familyId: 'BBBB000000000001' })],
...
describe('every observed execution gets its own vehicle', () => {
  ...
  it('keeps one vehicle per family when a session runs two different queries', () => {
    const roster = buildVehicleRoster({
      events: [
        ...
        event({ id: 'b', ordinal: 2, familyId: 'BBBB000000000001' }),
      ],
      families: [family(), family({ familyId: 'fam-2', familyId: 'BBBB000000000001' })],
  ...
  it('says so when no observed execution carried a query hash at all', () => {
    ...
    const roster = buildVehicleRoster({
      events: [event({ hashReported: false, familyId: null })],
      families: [family()],
      roads: [road()],
      blocked: noBlocks,
      now: NOW,
  ...
  it('distinguishes an old API from an engine that reported no hash', () => {
    const roster = buildVehicleRoster({
      events: [event({ hashReported: true, familyId: null })],
      families: [family()],
      roads: [road()],
      blocked: noBlocks,
      now: NOW,
...
describe('the roster says why it is empty, since six causes look identical on the map', () => {
  it.each([
    ...
    ['matched no query family', {
      events: [event({ familyId: 'FFFF000000000001' })], families: [family()], roads: [road()],
    }],
    ['names no road drawn', {
