 ...
 */
export function familyMeanDurationMs(family: OperationFamily): number | null {
...
export function buildVehicleRoster(input: VehicleInput): VehicleRoster {
  ...
  for (const event of events) {
    ...
    const family = event.familyId
      ...
      : (event.familyId ? familyByHash.get(event.familyId) : undefined)
    if (!family) {
      unmatchedHash += 1
      continue
    }
 ...
 */
export function vehicleClass(family: OperationFamily): VehicleClass {
