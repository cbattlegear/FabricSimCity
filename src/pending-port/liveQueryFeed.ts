...
export function advanceQueryFeed(
  ...
  const familyByHash = new Map<string, OperationFamily>()
  for (const family of families) {
    const key = normalizeHash(family.familyId)
  ...
  for (const request of snapshot.requests) {
    ...
    const hashReported = request.familyId !== undefined
    const familyId = normalizeHash(request.familyId ?? null)
    const family = familyId ? familyByHash.get(familyId) ?? null : null
  ...
  for (const event of arrived) {
    ...
    arrivedHashes.set(event.familyId, (arrivedHashes.get(event.familyId) ?? 0) + 1)
  }
  ...
  for (const completed of completedSample?.queries ?? []) {
    ...
    const familyId = normalizeHash(completed.familyId ?? null)
    let count = completed.executions
    if (familyId) {
      const overlap = arrivedHashes.get(familyId) ?? 0
      if (overlap > 0) {
        const applied = Math.min(overlap, count)
        count -= applied
        arrivedHashes.set(familyId, overlap - applied)
      }
    }
    ...
    arrived.push({
      ...
      familyId,
      familyId: family?.familyId ?? null,
      hashReported: completed.familyId !== undefined,
      blocked: false,
      waitType: null,
