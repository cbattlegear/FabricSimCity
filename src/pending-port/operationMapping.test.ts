...
function object(itemId: string): CapacityCityItem {
  return { itemId, name: itemId, workspaceId: 's', workspaceName: 's' } as unknown as CapacityCityItem
}
 ...
 * `shadowInvalidation.test.ts`, which pins a rule about `CapacityCityScene.ts` the same way.
 */
const view = readFileSync(fileURLToPath(new URL('./CapacityCityView.tsx', import.meta.url)), 'utf8')
...
describe('the families table filter is wired to the shared split', () => {
  it('renders the split rather than the raw family list', () => {
    ...
    expect(code).not.toContain('page.topOperationFamilies.map(family =>')
  })
  ...
  it('no longer claims a fixed top 12', () => {
    ...
    expect(code).toContain('Backend-ranked top {page.topOperationFamilies.length}')
  })
