 ...
 * that landed on two different cities, and clicking one database and then its neighbour to compare
 * them is an ordinary thing to do in the atlas. So this records what each click actually hit and
 * names a database only when both clicks of the pair agreed on it. A pair that disagreed, or that
...
export class CityActivation {
  ...
  /** Records a click on `databaseId`, or on the ground between cities when it is null. */
  click(databaseId: string | null): void {
    this.previousClick = this.lastClick
    this.lastClick = databaseId
  }

  /**
   * Returns the database the double-click should open, or null when its two clicks did not agree on
