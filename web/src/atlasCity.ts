...
export interface AtlasCityPlan {
   ...
   * the database city already makes when it draws a block.
   */
 ...
 */
export function townOutline(side: number, databaseId: string): AtlasPoint[] {
  const phases = OUTLINE_HARMONICS.map(
    (_, index) => ((stableHash(`${databaseId}:outline:${index}`) % 3600) / 3600) * Math.PI * 2,
  )
...
export function planAtlasCity(database: DatabaseAtlasItem): AtlasCityPlan {
  ...
  if (side === null) {
    return {
      ...
      towerHeight: null,
      columns: 0,
      outline: townOutline(UNKNOWN_SIDE, database.databaseId),
      radius: { min: 0, max: 0 },
      lots: [],
      streets: [],
      gateways: [],
   ...
   * of the ground its database paid for -- a lot counts as placed only when all four of its corners
   * are still inside the outline.
   */
  const spin = ((stableHash(`${database.databaseId}:spin`) % 900) / 900) * (Math.PI / 2)
  ...
  for (let row = 0; row < span; row += 1) {
    for (let column = 0; column < span; column += 1) {
      ...
      const key = `${database.databaseId}:jitter:${row}:${column}`
  ...
  const seeds = placed.map((_, index) => stableHash(`${database.databaseId}:${index}`))
 ...
 * from every other town's, and it was the same graph-paper look already removed from the database
...
export function townStreets(
  ...
  const count = 3 + (stableHash(`${databaseId}:radials`) % 4)
  const start = ((stableHash(`${databaseId}:radial-start`) % 3600) / 3600) * Math.PI * 2
  for (let index = 0; index < count; index += 1) {
    const skew = ((stableHash(`${databaseId}:radial:${index}`) % 1000) / 1000 - 0.5) * 0.55
    const angle = start + (index / count) * Math.PI * 2 + skew
    const reach = outlineRadiusAt(outline, angle) * RADIAL_ROAD_FRACTION
    const bow = (((stableHash(`${databaseId}:bow:${index}`) % 1000) / 1000) - 0.5) * 0.34
 ...
 * no streets -- which means a database whose allocated size is unknown -- keeps its centre, because
