...
export function placeIncident(
  ...
  const counterparts = new Set(counterpartObjectIds.filter(id => id !== objectId))

  const shared = roads
    .filter(road => road.polyline.length >= 2)
    .filter(
      road =>
        (road.fromObjectId === objectId && counterparts.has(road.toId))
        || (road.toId === objectId && counterparts.has(road.fromObjectId)),
    )
    .sort(byBusiest)[0]
  if (shared) {
  ...
  if (frontage) {
    const touching = roads
      .filter(road => road.polyline.length >= 2)
      .filter(road => road.fromObjectId === objectId || road.toId === objectId)
      .sort(byBusiest)[0]
    if (touching) {
