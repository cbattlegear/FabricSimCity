import type { DatabaseCityQueryFamily, DatabaseCityObject } from './databaseCityContracts'
 ...
 *
 * A family carries `objectIds`, and the obvious test is whether it carries any at all. That test
...
export function familyOnMap(
  family: Pick<DatabaseCityQueryFamily, 'objectIds'>,
  placedObjectIds: ReadonlySet<string>,
): boolean {
  return family.objectIds.some(id => placedObjectIds.has(id))
}

/** The ids of everything this city has actually placed, which is what a route can stop at. */
export function placedObjectIds(objects: readonly DatabaseCityObject[]): ReadonlySet<string> {
  return new Set(objects.map(object => object.objectId))
}
 ...
 */
export function splitQueryFamiliesByMap<T extends Pick<DatabaseCityQueryFamily, 'objectIds'>>(
  families: readonly T[],
